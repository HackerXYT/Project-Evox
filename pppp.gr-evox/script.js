                ...
                fetch('http://192.168.1.116:1974/reviews')
                    ...
                    .then(data => {
                        if (data.data) {
                            ...
                            document.getElementById("rating_average").innerHTML = `${average}/5.0`
                            ...
                            let selectedKey;
                            do {
                                selectedReview = dictionary_ratings[Math.floor(Math.random() * dictionary_ratings.length)];
                                selectedKey = selectedReview.name + "|" + selectedReview.review_gr; // unique key
                            } while (shownReviews.includes(selectedKey));

                            shownReviews.push(selectedKey);
                            localStorage.setItem("shownRatings", JSON.stringify(shownReviews));

                            //Show user's profile picture instead.
                            //document.getElementById("rating_icon").innerHTML = `<img src="${selectedReview.profile_picture}">`
                            rating.querySelector(".nameAndRating .name").innerText = selectedReview.name;
                            rating.querySelector(".nameAndRating .rate").innerText = selectedReview.review_gr;
                            rating.querySelector(".nameAndRating .rate").innerText = selectedReview.review_gr;
                    ...
                    .catch(error => {
                        if (!sessionStorage.getItem("continueAnyway")) document.getElementById("evxWarning").style.display = 'flex'
                    });
            } else {
                if (!sessionStorage.getItem("continueAnyway")) document.getElementById("evxWarning").style.display = 'flex'
            }
...
function goTo(event, container) {
    ...
    if (clicked.className === "item") {
        ...
        if (button === "Αρχική") {
            document.querySelector(".section.first").scrollIntoView({ behavior: 'smooth', block: 'start' });
        } else if (button === "Βιογραφικό") {
            learnMore()
        } else if (button === "Υπηρεσίες") {
            document.querySelector(".section.services").scrollIntoView({ behavior: 'smooth', block: 'start' });
