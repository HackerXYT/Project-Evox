...
function continueSetupAccount(doServer) {
  ...
  fetch(
    `https://arc.evoxs.xyz/?pin=${pin}&emri=${foundName}&metode=addInstagram${fetchQuery}`,
  )
    ...
    .then((status) => {
      if (status.message === "Complete") {
        ...
        alert(
          "Το PIN άλλαξε κατά την πορεία σύνδεσης, δοκιμάστε να συνδεθείτε ξανά.",
        );
      }
    })
    .catch((error) => {
...
let animateTransformSVG = `<svg version="1.1" xmlns="http://www.w3.org/2000/svg"
                xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" viewBox="0 0 50 50" width="17.5px"
                height="17.5px" style="enable-background:new 0 0 50 50;" xml:space="preserve">
                <path fill="#dedede"
...
function verifyEmail() {
  ...
  fetch(
    `https://arc.evoxs.xyz/?pin=${pin}&emri=${foundName}&metode=startEmail&email=${input.value}`,
  )
    .then((response) => response.json())
    .then((status) => {
      ...
      if (status.message === "Verification Started") {
      ...
      } else if (status.message === "Used.") {
        EvalertNext({
          title: "Αποτυχία επαλήθευσης Email",
          description:
            "Το email που εισάγατε χρησιμοποιείται ήδη από άλλο λογαριασμό.<br>Παρακαλώ χρησιμοποιήστε ένα διαφορετικό email.",
          buttons: ["Συνέχεια"],
          buttonAction: [
            "document.getElementById('voxEmail-input').value = ''",
          ],
          addons: [],
        });
        ...
        EvalertNext({
          ...
          description:
            "Ο διακομιστής έχει υπερφορτωθεί και το Email δε μπόρεσε να σταλθεί.<br>Θα χρειαστεί να επιβεβαιώσετε το email σας αργότερα.",
          buttons: ["Συνέχεια"],
...
function completeVerification() {
  ...
  fetch(
    `https://arc.evoxs.xyz/?pin=${pin}&emri=${foundName}&metode=verifyEmail&email=${code}`,
  )
    .then((response) => response.json())
    .then((status) => {
      ...
      if (status.message === "Complete") {
        ...
        EvalertNext({
          title: "Λάθος κωδικός",
          description:
            "Ο κωδικός που πληκτρολογήσατε δεν είναι σωστός.<br>Έλεγξτε ξανά τον κωδικό που στάλθηκε στο email σας και πληκτρολογήστε τον παρακάτω.",
          buttons: ["Συνέχεια"],
          buttonAction: [],
          addons: [],
        });
